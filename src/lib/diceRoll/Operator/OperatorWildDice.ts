...
export default class OperatorWildDice extends Operator {
  ...
  executeOperation(dices: number[], dice: Dice): Operation[] {
    ...
    const resSubstractOperation = operatorRerollSubstract.executeOperation(filteredDices.map((value) => value.value), dice);
    ...
    const filteredResult = resSubstractOperation.filter((value, index) => !value.dropped || (index > 0 && !resSubstractOperation[index - 1].dropped));

    // Execute explode operation on filtered result
    const resExplodeOperation = operatorExplode.executeOperation(filteredResult.map((value) => value.value), dice);
