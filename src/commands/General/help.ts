...
export class HelpCommand extends Command {
	 ...
	 */
	private async buildDefaultEmbedMessage(filter: (cmd: Command<Args, CommandOptions>) => PreconditionContainerReturn): Promise<EmbedBuilder> {
	...
	private buildDetailEmbedMessage(command: string): EmbedBuilder | undefined {
		...
		const options = typeof cmd.detailedDescription === "string" ? { description: cmd.detailedDescription.toString() } : cmd.detailedDescription;
